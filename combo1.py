...
class ChatBot:
    def __init__(self, excel_file='input/BOT Catalogue.xlsx'):
        self.responses = {}
        try:
        ...
        # Create a separate DataFrame to store preprocessed text for matching
    ...
    def is_gibberish(self, input_text):
        ...
        return any(word.lower() not in words for word in input_words)
    ...
    def preprocess_excel_data(self):
        """Store preprocessed text separately for matching purposes."""
        for column in self.columns_to_search:
            if column in self.df.columns:
                self.preprocessed_df[column] = self.df[column].apply(self.preprocess_text)

    def get_response(self, user_input):
        return_respond_to_json = self.respond_to_json(user_input)
        if return_respond_to_json:
            return return_respond_to_json
        elif user_input.lower() and len(user_input) < 3:
    ...
    def get_exact_match_response(self, user_input):
        ...
        for column in self.columns_to_search:
            if column in self.df.columns:
                # Use preprocessed text for matching
                exact_result = self.preprocessed_df[self.preprocessed_df[column].apply(
                ...
                if not exact_result.empty:
                    ...
                    # Retrieve original text from the main DataFrame
                    for idx in exact_result.index:
                        row = self.df.loc[idx]
    ...
    def get_partial_match_response(self, user_input):
        ...
        for column in self.columns_to_search:
            if column in self.df.columns:
                # Use preprocessed text for matching
                result = self.preprocessed_df[self.preprocessed_df[column].apply(
        ...
        if not matches.empty:
            if len(matches) > 30:
                ...
                # Retrieve original text from the main DataFrame for matching rows
